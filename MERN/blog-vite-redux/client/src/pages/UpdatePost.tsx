...
const UpdatePost = () => {
  ...
  const [imageUploadProgress, setImageFileUploadingProgress] =
    useState<number | null>(null)
  const [imageFileUrl, setImageFileUrl] = useState<string | null>(
    null
  )
  const [imageUploadError, setImageFileUploadError] = useState<
    string | null
  >(null)
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await axios.get(`${postApi}?postId=${postId}`)
        if (res.status === 200) {
  ...
  const handleSubmit = async (
    ...
    try {
      const res = await axios.put(
        `${postApi}/${postId}/${currentUser?._id}`,
        formData,
        {
